...
var currentFileFolderType  = '';
var currentFileFolderLevel = '';
...
(function () {
    ...
    Cnds.prototype.ForEachFileFolder = function (path, type, recursive) {
      ...
      var walk = function (dir, level = 0) {
        var results = [];
        var list    = fs.readdirSync(dir);
        list.forEach(function (file) {
          let fullFilePath     = epath.join(dir, file);
					...
					if (exist) {
						if ((type === 0 && folder) || (type === 1 && !folder) || type === 2) {
							currentFileFolder      = file;
							currentFileFolderType  = folder ? 'folder' : 'file';
    ...
    Exps.prototype.CurrentFileFolderType = function (ret) {
      if (!runningElectron)
        return;
      ret.set_any(currentFileFolderType);
    };

    Exps.prototype.CurrentFileFolderLevel = function (ret) {
      if (!runningElectron)
        return;
      ret.set_any(currentFileFolderLevel);
    };
